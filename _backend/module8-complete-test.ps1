...
$p=@($projects.data.projects|Where-Object{$_.id})[0];if(!$p){Fail 'Dynamic project selection' 'No accessible project'}else{Pass 'Dynamic project selection'}
$sites=Api GET ("/api/sites?project_id="+$p.id);$s=@($sites.data.sites|Where-Object{$_.id})[0];if(!$s){Fail 'Dynamic site selection' 'No site'}else{Pass 'Dynamic site selection'}
$zones=Api GET ("/api/site-zones?site_id="+$s.id);$z=@($zones.data.zones|Where-Object{$_.id})[0];if(!$z){Fail 'Dynamic zone selection' 'No zone'}else{Pass 'Dynamic zone selection'}
$boqHeaders=Api GET ("/api/project-boqs?project_id="+$p.id);$bh=@($boqHeaders.data.project_boqs|Where-Object{$_.id})[0];if(!$bh){Fail 'Dynamic BOQ header selection' 'No BOQ header'}else{Pass 'Dynamic BOQ header selection'};$boqs=Api GET ("/api/project-boqs/"+$bh.id+"/items");$b=@($boqs.data.boq_items|Where-Object{[int]$_.site_id-eq[int]$s.id})[0];if(!$b){Fail 'Dynamic BOQ selection' 'No BOQ item'}else{Pass 'Dynamic BOQ selection'}
