...
export function ProcurementGoodsReceiptPage() {
  ...
  useEffect(() => {
    try {
      const saved = localStorage.getItem('mock_procurement_ProcurementGoodsReceiptPage');
      if (saved) {
  ...
  useEffect(() => {
    ...
    if (receipts.length > 0 || saved) {
       localStorage.setItem('mock_procurement_ProcurementGoodsReceiptPage', JSON.stringify(receipts));
    }
  }, [receipts]);
  // ---------------------------------
